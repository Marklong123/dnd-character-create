...
import { formatModifier, spellAttackBonus, spellSaveDC, feetToMeters } from '@/utils/calculations'
import { usePdfExport } from '@/composables/usePdfExport'
import { copyShareUrl } from '@/utils/shareCharacter'
import { SKILLS } from '@/data/dnd5e/skills'
import { getBackgrounds, getClasses, getRaces, getSpells } from '@/data'
...
const subclassName = computed(() => {
  ...
  const cls = variantClasses.value.find(c => c.id === char.value.className)
  const subclass = cls?.subclasses.find(s => s.id === char.value.subclass)
  if (!subclass) return gt.subclassName(char.value.subclass)
...
const displayBackground = computed(() => {
  ...
  return (bg as any).nameOriginal || gt.background(bg.name)
})
...
const hitDiceDisplay = computed(() => `${char.value.level}d${char.value.hitDie}`)
const exportValidationErrors = computed(() => validateCharacterForExport(char.value))
const canExportProjectInfinityJson = computed(() => exportValidationErrors.value.length === 0)
...
function gp(amount: number | null | undefined): string {
...
<template>
  <section aria-labelledby="review-heading">
    ...
    <div v-if="exportValidationErrors.length" class="bg-red-950/40 border border-red-700 rounded-lg p-4 mb-4">
      <h3 class="font-semibold text-red-300 mb-2">{{ t('review.exportBlocked') }}</h3>
      <ul class="list-disc list-inside text-sm text-red-200 space-y-1">
        <li v-for="error in exportValidationErrors" :key="error">{{ error }}</li>
      </ul>
    </div>
    ...
    <div class="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-4">
      <div v-for="a in abilities" :key="a" class="bg-stone-800 border border-stone-700 rounded-lg p-3 text-center">
      ...
      <div class="bg-stone-800 border border-stone-700 rounded-lg p-4">
        ...
        <div class="space-y-1 text-sm">
          <div v-for="st in savingThrows" :key="st.ability" class="flex items-center gap-2">
            <span class="w-3 h-3 rounded-full" :class="st.proficient ? 'bg-amber-500' : 'bg-stone-600'"></span>
            <span class="text-stone-400 w-12">{{ gt.ability(st.ability) }}</span>
      ...
      <div class="bg-stone-800 border border-stone-700 rounded-lg p-4">
        ...
        <div class="space-y-1 text-xs max-h-60 overflow-y-auto">
          <div v-for="skill in skills" :key="skill.id" class="flex items-center gap-2">
            <span class="w-2.5 h-2.5 rounded-full" :class="skill.proficient ? 'bg-amber-500' : 'bg-stone-600'"></span>
    ...
    <div v-if="char.spellcastingAbility" class="bg-stone-800 border border-stone-700 rounded-lg p-4 mb-4">
      ...
      <div v-if="char.spellsKnown.length" class="mb-2">
        <span class="text-xs text-stone-500">{{ char.className === 'wizard' ? t('spells.spellbook') : t('spells.knownSpells') }}:</span>
        <span class="text-stone-300 text-sm ml-1">{{ char.spellsKnown.map(spellName).join(', ') }}</span>
      </div>
      <div v-if="char.spellsPrepared.length">
        <span class="text-xs text-stone-500">{{ t('spells.preparedSpells') }}:</span>
        <span class="text-stone-300 text-sm ml-1">{{ char.spellsPrepared.map(spellName).join(', ') }}</span>
      </div>
    </div>
    ...
    <div v-if="char.weapons.length || char.armor || char.equipment.length" class="bg-stone-800 border border-stone-700 rounded-lg p-4 mb-4">
      ...
      <div v-if="char.equipment.length">
        ...
        <p class="text-stone-400 text-sm">{{ char.equipment.map(item => gt.equipment(item)).join(', ') }}</p>
      </div>
      <p class="text-xs text-stone-500 mt-3">
        {{ t('equipment.spent') }}: {{ gp(char.equipmentSpentGp) }} / {{ t('equipment.budget') }}: {{ gp(char.equipmentBudgetGp) }}
      </p>
    </div>
    ...
    <Transition name="fade">
      <div v-if="saveMessage" class="mt-4 p-3 rounded-lg border flex items-center gap-3 bg-green-900/30 border-green-700 text-green-300" role="status">
        <p class="flex-1 text-sm">{{ saveMessage.text }}</p>
      </div>
    </Transition>
    <Transition name="fade">
      <div v-if="shareMessage" class="mt-4 p-3 rounded-lg border flex items-center gap-3"
        :class="shareMessage.type === 'success' ? 'bg-blue-900/30 border-blue-700 text-blue-300' : 'bg-red-900/30 border-red-700 text-red-300'"
        role="status">
        <div class="flex-1">
    ...
    <div class="flex flex-wrap gap-3 mt-6 no-print" role="group" :aria-label="t('review.export')">
      <button @click="exportPdf" :disabled="exporting" class="px-6 py-2 bg-amber-600 hover:bg-amber-500 text-stone-900 font-semibold rounded-lg transition-colors disabled:opacity-50 cursor-pointer">
        {{ exporting ? t('common.loading') : t('review.exportPdf') }}
      </button>
      <button @click="downloadJson" :disabled="!canExportProjectInfinityJson"
        class="px-6 py-2 rounded-lg transition-colors cursor-pointer"
        :class="canExportProjectInfinityJson ? 'bg-stone-700 hover:bg-stone-600 text-stone-200' : 'bg-stone-800 text-stone-600 cursor-not-allowed'">
        {{ t('review.exportJson') }}
      </button>
      <button @click="saveChar" class="px-6 py-2 bg-green-700 hover:bg-green-600 text-stone-200 rounded-lg transition-colors cursor-pointer">
        {{ t('review.save') }}
      </button>
      <button @click="printSheet" class="px-6 py-2 bg-stone-700 hover:bg-stone-600 text-stone-200 rounded-lg transition-colors cursor-pointer">
        {{ t('review.print') }}
      </button>
      <button @click="shareCharacter" class="px-6 py-2 bg-blue-700 hover:bg-blue-600 text-stone-200 rounded-lg transition-colors cursor-pointer">
        {{ t('review.shareUrl') }}
      </button>
    </div>
